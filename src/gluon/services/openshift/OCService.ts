...
export class OCService {
    ...
    public async addTeamMembershipPermissionsToProject(projectId: string, team: QMTeam) {
        const teamOwners = team.owners.map( owner => getUsernameFromDomainUsername(owner.domainUsername) );
        ...
        const teamMembers = team.members.map( owner => getUsernameFromDomainUsername(owner.domainUsername) );
    ...
    public async removeTeamMembershipPermissionsFromProject(projectId: string, domainUserName: string) {
        const memberUsername = getUsernameFromDomainUsername(domainUserName);
