...
def label = "dotnet22-${UUID.randomUUID().toString()}"
...
echo "Starting to try find pod template: " + dockerRegistryIp + '/' + sharedResourceNamespace + '/jenkins-slave-dotnet22-subatomic:2.0'

podTemplate(cloud: "openshift", label: label, serviceAccount:"jenkins", containers: [
    containerTemplate(name: 'jnlp', image: dockerRegistryIp + '/' + sharedResourceNamespace + '/jenkins-slave-dotnet22-subatomic:2.0', ttyEnabled: false, alwaysPullImage: true, args: '${computer.jnlpmac} ${computer.name}')
  ])
{
    ...
    node(label) {
        ...
        def tag
        stage('Run dot net stage') {
            container('jnlp') {
                stage('Run Dot Net Checks and Tests') {
